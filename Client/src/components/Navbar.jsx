                ...
                <nav className={`py-4 lg:px-24 px-4 ${isSticky ? "sticky top-0 left-0 right-0 bg-blue-300": ""}`} >
                    <div className='flex justify-between items-center text-base gap-8'>
                        ...
                        <div className="md:hidden">
                            <button onClick={toggleMenu} className="text-balck focus:outline-none">
                                {
                                    isMenuOpen ? <FaXmark className="w-5 hover:text-black"/> : <FaBarsStaggered className="w-5 hover:text-black"/>
                                }
                            </button>
                        </div>
                    </div>
                    ...
                    <div className={`space-y-4 px-4 mt-12 py-7 bg-blue-700 ${isMenuOpen ? "block fixed top-0 right-0 left-0" :"hidden"} `}>
                        {navItems.map(({ link, path }) => (
                            ...
                            text-white uppercase cursor-pointer ${isMenuOpen ? "block" : "hidden"}`}>{link}</Link>
                        ))}
                    </div>
                </nav>
            </header>
